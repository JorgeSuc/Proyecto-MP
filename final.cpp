...
string padding(string oracio){
	while(oracio.length()%8!=0){
		oracio+='*';
	}
	return oracio;
}
...
string tobyte(string palabrita){
	string s;
	for(int i=0;i<palabrita.length();i++){
		s+=converttobinary(palabrita[i]);
		
	}
	return s;
}

void split(string palabrita, string &izqui, string &derecha){
	int n=palabrita.length()/2;
	for(int i=0;i<n;i++){
		izqui+=palabrita[i];
		derecha+=palabrita[i+n];
	}
}
...
string merge(string izqui, string derecha){
	return izqui + derecha;
}
...
string s_box(string palabrita){
	string s;
	for(int i=0;i<48;i=i+6){
		string a,b;
		a=palabrita[i];a+=palabrita[i+5];
		int a1 = converttodec(a);
		b=palabrita[i+1];b+=palabrita[i+2];b+=palabrita[i+3];b+=palabrita[i+4];
...
main(){
	string text;
	string key;
	cout<<"Texto a ingresar : ";cin>>text;
	cout<<"Key de 8 caracteres : ";cin>>key;
	...
	for(int i=0;i<16;i++){
		...
		cout<<"Round Result\t"<<samayac+  "\t "<<endl <<endl;
	}
